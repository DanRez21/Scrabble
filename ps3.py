    ...
    * The hand finishes when there are no more unused letters.
    ...
    while calculate_handlen(hand) != 0:    
        ...
        if c == '!!':
            ...
            if is_valid_word(c, hand, word_list):
                sums += get_word_score(c, calculate_handlen(hand))
                print(f'"{c}" earned {get_word_score(c, calculate_handlen(hand))} points. Total: {sums} points')
            else:
...
def substitute_hand(hand, letter):
    ...
    """
    hand_copy = copy.deepcopy(hand)
    if letter in hand_copy.keys() and letter != '*':
        for rand_let in random.choice(string.ascii_lowercase):
            if rand_let not in hand_copy.keys():
                hand_copy[rand_let], hand_copy[letter] = hand_copy[letter], 0
                break
    return hand_copy
    ...
    * Returns the total score for the series of hands
    ...
    while True:
        try:
            ...
            if num_of_hands > 0: 
                break          
            print("Please, enter number of hands over 0")
    while num_of_hands > 0:
        ...
        print('-'*13)
        if repeat == 0:
            if check("Would you like to replay the hand? ") == 'no':
                total += sums
                num_of_hands -= 1
            else:
                sum_1 = sums
            ...
            repeat = 0
            total += max(sums, sum_1)
            num_of_hands -= 1
    print(f"Total score over all hands: {total} points")
